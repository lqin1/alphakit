...
**照着这份文档可以把引擎重建出来。** 架构总览与设计理由在
[`architecture.md`](architecture.md)（先读那份，十分钟）；数据契约在
[`l2_schema.md`](l2_schema.md)；操作说明在 [`manual.md`](manual.md)；
**尚未实现**的部分在 [`roadmap.md`](roadmap.md)。

没有标记的章节都是**已实现的契约**；`[TARGET]` 标记的是目标架构，不要照着实现。
同一件事若本文与 `l2_schema.md` 都提到，**以 `l2_schema.md` 为准**——那边是契约，这边是规格。

**从头重建的读法**：先读 §〇 的不变量清单与两个可手算的例子（在 §6.2 之后），
它们是你写完代码后逐位对照的基准；再按 §三 → §四 → §六 → §七 → §八 → §九 的顺序实现。

| 章 | 内容 | 重建时的分量 |
|---|---|---|
| 〇 | **不变量清单 + 验收基准** | **先读** |
| 一 | 系统总览 | 背景（详见 `architecture.md`） |
| 二 | 仓库拓扑、模块清单、region 与可比性 | 必读 |
| 三 | L3 命名与路径（**含折叠规则**）、秩、两道掩码闸门 | **核心** |
| 四 | Node 模型：yaml schema、outputs 与返回值规则、命名约定 | **核心** |
| 五 | L2/L1 接入 `[TARGET]` | 跳过 |
| 六 | ctx 的 API 面；**ops 精确语义 + 可手算的例子**（§6.2）；**指纹**（§6.3） | **核心** |
| 七 | 执行引擎：逐日主循环、预热、新鲜度回退 | **核心** |
| 八 | 仿真内核（§8.2）、交付物字段规格（§8.4b）、七道闸门（§8.5） | **核心** |
| 九 | 退市与停牌：NaN 三分类 | **核心** |
| 十 | Ctx 的实现约束 | 必读 |
| 十一 | 质量防线 `[TARGET]` | 跳过 |
| 十二 | CLI 参考 | 必读 |
| 十三–十五 | 测试验收、选型、研究工作流 | 背景 |
