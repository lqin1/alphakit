...
`-e` 是可编辑安装：研究期你会改引擎代码，改完不必重装。

**依赖与下界**——下界是实测跑通的版本，不是虚报的兼容范围，装低了不是慢是**错**：

| | 下界 | 为什么是这个下界 |
|---|---|---|
| Python | 3.11 | 不用 3.12 的语法（PEP 701 的 f-string 等），CI 在 3.11/3.13 双跑 |
| `pandas` | 3.0.5 | 依赖它的 copy-on-write 语义——ops 链「永远返回副本」的前提 |
| `zarr` | 3.3.0 | L3 用的是 zarr **v3** 布局 |
| `pyarrow` | 25.0.1 | `--weight` 读 feather/parquet 时才用到 |
| `PyYAML` | 6.0.3 | config 解析 |

装上四个 console script：`alphakit` / `ak` → `alpha_kit.cli:main`，
`run` → `main_run`，`pnl` → `main_pnl`。打包只收 `alpha_kit*` 四个包——
`repos/`、`pipeline/`、`storage/`、`tests/` 都带 `.py`，自动发现会把研究仓库和
几十 MB 数据一起打进 wheel。
...
> **500 个文件、约 25 万行**全部显示为改动, 而数据本身逐字节相同——你会得到一棵看不出
...
1. **ops 链必须以 `scale` 收尾**。少了它，上游各自 `Σ|w|=1` 的权重线性组合后会因方向相反处
   互相抵消而缩水。**本仓库真实跑出来的三个 alpha 按 0.4/0.3/0.3 混合，`Σ|w|` 只剩 0.5088（末日；全区间均值 0.526、区间 [0.436, 0.645]）**
...
**每次 `run` 都会先做一遍零数据的预检**（约 10 ms，只查元数据、不读任何面板），
...
免费源。前三条写在 `_meta.json` 的 `known_defects` 里（`survivorship_bias_no_delisted` /
`no_vwap` / `adj_factor_not_pit`），其余是同样已知、但未编码成字段的限制；`pnl` 另写一份
4 项的清单进 `metrics.json`。一律**不做静默修补**：
